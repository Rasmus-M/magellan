...
public class MagellanImportExport {
    ...
    protected int readDataFile(File mapDataFile) {
        ...
        try {
            ...
            int currMap = 0;
            int spriteRead = TIGlobals.MINSPRITE;
            int spriteRead1 = TIGlobals.MINSPRITE;
            int spriteRead2 = TIGlobals.MINSPRITE;
            int spritePalNo = 0;
            int spriteColNo = 0;
            do {
                lineIn = br.readLine();
                ...
                if (indexColorModel.getMapSize() == 16) {
                    ...
                    for (int sy = 0; sy < ySprites; sy++) {
                        ...
                        for (int sx = 0; sx < xSprites; sx++) {
                            int x0 = sx * size;
                            if (spriteIndex <= TIGlobals.MAXSPRITE) {
    ...
    protected void writeDataFile(File mapDataFile) {
        try {
            ...
            bw.newLine();
            bw.write(Globals.KEY_CHARRANG + TIGlobals.MINCHAR + "|" + +TIGlobals.MAXCHAR);
            bw.newLine();
            // save colorsets
            bw.write("* COLORSET");
            bw.newLine();
    ...
    protected void writeXBDataFile(File mapDataFile, int startChar, int endChar, int codeLine, int charLine, int mapLine, int interLine, int exportType, boolean includeComments, boolean currMapOnly, boolean excludeBlank) {
        ...
        try {
            ...
            if (exportType == Globals.BASIC_PROGRAM) {
                ...
                colorSetNum = (int) (Math.floor((startChar - TIGlobals.FIRSTCHAR) / 8)) + 1;
...
//            }
    ...
    protected void writeBinaryMap(File mapDataFile) {
        try {
            // store working map first
            mapdMain.storeCurrentMap();
            // get file output buffer
